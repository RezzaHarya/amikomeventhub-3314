...
@section('content')
    ...
    <div class="bg-white rounded-[2.5rem] border border-slate-100 shadow-sm overflow-hidden mt-8">
        <div class="px-8 py-6 bg-slate-50/50 border-b flex justify-between items-center gap-4">
            ...
            <a href="{{ route('admin.partners.create') }}" class="px-6 py-3 bg-indigo-600 text-white font-bold rounded-xl hover:bg-indigo-700 transition shadow-sm flex items-center gap-2">
            ...
            <table class="w-full text-left border-collapse">
                ...
                <tbody class="divide-y border-t">
                    ...
                    <tr class="hover:bg-slate-50/50 transition">
                        ...
                        <td class="px-8 py-6">
                            @if($partner->logo_url)
                                <a href="{{ $partner->logo_url }}" target="_blank" class="text-indigo-500 hover:underline text-sm">Lihat Logo</a>
                            @else
                                <span class="text-slate-400 text-sm">Tidak ada logo</span>
                            @endif
                        </td>
                        <td class="px-8 py-6">
                            <div class="flex gap-2 justify-center">
                                ...
                                <form action="{{ route('admin.partners.destroy', $partner->id) }}" method="POST" onsubmit="return confirm('Yakin ingin menghapus partner {{ $partner->name }}?');">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit"
                    ...
                    @empty
                    <tr>
                        <td colspan="4" class="px-8 py-6 text-center text-slate-500 font-medium">Belum ada data partner.</td>
                    </tr>
                    @endforelse
                </tbody>
            </table>
